...
begin
 ...
 Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'wboit_frag.spv');
 try
 ...
 Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'wboit_alphatest_frag.spv');
 try
...
//fFrameGraph.VulkanDevice.DebugMarker.SetObjectName(fParticleVertexShaderModule.Handle,TVkDebugReportObjectTypeEXT.VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,'fParticleVertexShaderModule');
 ...
 if fInstance.Renderer.Scene3D.RaytracingActive then begin
  Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_raytracing_wboit_frag.spv');
 end else begin
  ...
  for PrimitiveTopology:=Low(TpvScene3D.TPrimitiveTopology) to High(TpvScene3D.TPrimitiveTopology) do begin
   ...
   for FaceCullingMode:=Low(TpvScene3D.TFaceCullingMode) to High(TpvScene3D.TFaceCullingMode) do begin
    ...
    try
     ...
     VulkanGraphicsPipeline.ViewPortState.AddViewPort(0.0,0.0,fResourceAccumulation.Width,fResourceAccumulation.Height,0.0,1.0);
     VulkanGraphicsPipeline.ViewPortState.AddScissor(0,0,fResourceAccumulation.Width,fResourceAccumulation.Height);
