 ...
 *                                                                            *
 ******************************************************************************)
unit PasVulkan.Scene3D.Renderer.Passes.LoopOrderIndependentTransparencyPass1RenderPass;
{$i PasVulkan.inc}
{$ifndef fpc}
...
type { TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass1RenderPass }
     TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass1RenderPass=class(TpvFrameGraph.TRenderPass)
      private
...
{ TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass1RenderPass }

constructor TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass1RenderPass.Create(const aFrameGraph:TpvFrameGraph;const aInstance:TpvScene3DRendererInstance);
begin
inherited Create(aFrameGraph);

 fInstance:=aInstance;

 Name:='LoopOrderIndependentTransparencyPass1RenderPass';
...
destructor TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass1RenderPass.Destroy;
begin
 inherited Destroy;
end;

procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass1RenderPass.AcquirePersistentResources;
...
//fFrameGraph.VulkanDevice.DebugMarker.SetObjectName(fMeshVertexShaderModule.Handle,TVkDebugReportObjectTypeEXT.VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,'fMeshVertexShaderModule');
 ...
 if fInstance.Renderer.SurfaceSampleCountFlagBits=TVkSampleCountFlagBits(VK_SAMPLE_COUNT_1_BIT) then begin
  if fInstance.ZFar<0.0 then begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'reversedz_shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'loopoit_pass1_frag.spv');
  end else begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'loopoit_pass1_frag.spv');
  end;
 end else begin
  if fInstance.ZFar<0.0 then begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'reversedz_shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'msaa_loopoit_pass1_frag.spv');
  end else begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'msaa__loopoit_pass1_frag.spv');
  end;
 end;
 try
...
//fFrameGraph.VulkanDevice.DebugMarker.SetObjectName(fMeshFragmentShaderModule.Handle,TVkDebugReportObjectTypeEXT.VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,'fMeshFragmentShaderModule');
 ...
 if fInstance.Renderer.SurfaceSampleCountFlagBits=TVkSampleCountFlagBits(VK_SAMPLE_COUNT_1_BIT) then begin
  if fInstance.ZFar<0.0 then begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'reversedz_shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'loopoit_pass1_alphatest_frag.spv');
  end else begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'loopoit_pass1_alphatest_frag.spv');
  end;
 end else begin
  if fInstance.ZFar<0.0 then begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'reversedz_shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'msaa_loopoit_pass1_alphatest_frag.spv');
  end else begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'msaa_loopoit_pass1_alphatest_frag.spv');
  end;
 end;
 try
...
//fFrameGraph.VulkanDevice.DebugMarker.SetObjectName(fParticleVertexShaderModule.Handle,TVkDebugReportObjectTypeEXT.VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,'fParticleVertexShaderModule');
 ...
 if fInstance.Renderer.Scene3D.RaytracingActive then begin
  if fInstance.Renderer.SurfaceSampleCountFlagBits=TVkSampleCountFlagBits(VK_SAMPLE_COUNT_1_BIT) then begin
   if fInstance.ZFar<0.0 then begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_raytracing_reversedz_loopoit_pass1_frag.spv');
   end else begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_raytracing_loopoit_pass1_frag.spv');
   end;
  end else begin
   if fInstance.ZFar<0.0 then begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_raytracing_reversedz_msaa_loopoit_pass1_frag.spv');
   end else begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_raytracing_msaa_loopoit_pass1_frag.spv');
   end;
  end;
 end else begin
  if fInstance.Renderer.SurfaceSampleCountFlagBits=TVkSampleCountFlagBits(VK_SAMPLE_COUNT_1_BIT) then begin
   if fInstance.ZFar<0.0 then begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_reversedz_loopoit_pass1_frag.spv');
   end else begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_loopoit_pass1_frag.spv');
   end;
  end else begin
   if fInstance.ZFar<0.0 then begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_reversedz_msaa_loopoit_pass1_frag.spv');
   end else begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_msaa_loopoit_pass1_frag.spv');
   end;
  end;
 end;
 try
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass1RenderPass.ReleasePersistentResources;
begin
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass1RenderPass.AcquireVolatileResources;
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass1RenderPass.ReleaseVolatileResources;
var Index:TpvSizeInt;
    AlphaMode:TpvScene3D.TMaterial.TAlphaMode;
    PrimitiveTopology:TpvScene3D.TPrimitiveTopology;
    FaceCullingMode:TpvScene3D.TFaceCullingMode;
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass1RenderPass.Update(const aUpdateInFlightFrameIndex,aUpdateFrameIndex:TpvSizeInt);
begin
 inherited Update(aUpdateInFlightFrameIndex,aUpdateFrameIndex);
end;

procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass1RenderPass.OnSetRenderPassResources(const aCommandBuffer:TpvVulkanCommandBuffer;
                                                                                                           const aPipelineLayout:TpvVulkanPipelineLayout;
                                                                                                           const aRendererInstance:TObject;
                                                                                                           const aRenderPass:TpvScene3DRendererRenderPass;
                                                                                                           const aPreviousInFlightFrameIndex:TpvSizeInt;
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass1RenderPass.Execute(const aCommandBuffer:TpvVulkanCommandBuffer;
