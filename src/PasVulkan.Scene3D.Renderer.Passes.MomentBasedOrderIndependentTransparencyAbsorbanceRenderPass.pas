...
begin
 ...
 Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'mboit_pass1_frag.spv');
 try
 ...
 Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'mboit_pass1_alphatest_frag.spv');
 try
...
//fFrameGraph.VulkanDevice.DebugMarker.SetObjectName(fParticleVertexShaderModule.Handle,TVkDebugReportObjectTypeEXT.VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,'fParticleVertexShaderModule');
 ...
 if fInstance.Renderer.Scene3D.RaytracingActive then begin
  Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_raytracing_mboit_pass1_frag.spv');
 end else begin
  Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_mboit_pass1_frag.spv');
 end;
 try
...
begin
 ...
 fPassVulkanDescriptorPool.AddDescriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,2*fInstance.Renderer.CountInFlightFrames);
  fPassVulkanDescriptorPool.AddDescriptorPoolSize(VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT,1*fInstance.Renderer.CountInFlightFrames);
  ...
  for PrimitiveTopology:=Low(TpvScene3D.TPrimitiveTopology) to High(TpvScene3D.TPrimitiveTopology) do begin
   ...
   for FaceCullingMode:=Low(TpvScene3D.TFaceCullingMode) to High(TpvScene3D.TFaceCullingMode) do begin
    ...
    try
     ...
     VulkanGraphicsPipeline.ViewPortState.AddViewPort(0.0,0.0,fResourceMoments0.Width,fResourceMoments0.Height,0.0,1.0);
     VulkanGraphicsPipeline.ViewPortState.AddScissor(0,0,fResourceMoments0.Width,fResourceMoments0.Height);
