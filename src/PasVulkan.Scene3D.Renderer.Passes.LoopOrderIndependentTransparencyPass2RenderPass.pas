 ...
 *                                                                            *
 ******************************************************************************)
unit PasVulkan.Scene3D.Renderer.Passes.LoopOrderIndependentTransparencyPass2RenderPass;
{$i PasVulkan.inc}
{$ifndef fpc}
...
type { TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass2RenderPass }
     TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass2RenderPass=class(TpvFrameGraph.TRenderPass)
      private
...
constructor TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass2RenderPass.Create(const aFrameGraph:TpvFrameGraph;const aInstance:TpvScene3DRendererInstance);
begin
...
destructor TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass2RenderPass.Destroy;
begin
 inherited Destroy;
end;

procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass2RenderPass.AcquirePersistentResources;
var Stream:TStream;
...
//fFrameGraph.VulkanDevice.DebugMarker.SetObjectName(fMeshVertexShaderModule.Handle,TVkDebugReportObjectTypeEXT.VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,'fMeshVertexShaderModule');
 ...
 if fInstance.Renderer.SurfaceSampleCountFlagBits=TVkSampleCountFlagBits(VK_SAMPLE_COUNT_1_BIT) then begin
  if fInstance.ZFar<0.0 then begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'reversedz_shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'loopoit_pass2_frag.spv');
  end else begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'loopoit_pass2_frag.spv');
  end;
 end else begin
  if fInstance.ZFar<0.0 then begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'reversedz_shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'msaa_loopoit_pass2_frag.spv');
  end else begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'msaa_loopoit_pass2_frag.spv');
  end;
 end;
 try
...
//fFrameGraph.VulkanDevice.DebugMarker.SetObjectName(fMeshFragmentShaderModule.Handle,TVkDebugReportObjectTypeEXT.VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,'fMeshFragmentShaderModule');
 ...
 if fInstance.Renderer.SurfaceSampleCountFlagBits=TVkSampleCountFlagBits(VK_SAMPLE_COUNT_1_BIT) then begin
  if fInstance.ZFar<0.0 then begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'reversedz_shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'loopoit_pass2_alphatest_frag.spv');
  end else begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'loopoit_pass2_alphatest_frag.spv');
  end;
 end else begin
  if fInstance.ZFar<0.0 then begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'reversedz_shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'msaa_loopoit_pass2_alphatest_frag.spv');
  end else begin
   Stream:=pvScene3DShaderVirtualFileSystem.GetFile('mesh_'+fInstance.Renderer.MeshFragTypeName+'shading_'+fInstance.Renderer.MeshFragGlobalIlluminationTypeName+'msaa_loopoit_pass2_alphatest_frag.spv');
  end;
 end;
 try
...
//fFrameGraph.VulkanDevice.DebugMarker.SetObjectName(fParticleVertexShaderModule.Handle,TVkDebugReportObjectTypeEXT.VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,'fParticleVertexShaderModule');
 ...
 if fInstance.Renderer.Scene3D.RaytracingActive then begin
  if fInstance.Renderer.SurfaceSampleCountFlagBits=TVkSampleCountFlagBits(VK_SAMPLE_COUNT_1_BIT) then begin
   if fInstance.ZFar<0.0 then begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_raytracing_reversedz_loopoit_pass2_frag.spv');
   end else begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_raytracing_loopoit_pass2_frag.spv');
   end;
  end else begin
   if fInstance.ZFar<0.0 then begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_raytracing_reversedz_msaa_loopoit_pass2_frag.spv');
   end else begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_raytracing_msaa_loopoit_pass2_frag.spv');
   end;
  end;
 end else begin
  if fInstance.Renderer.SurfaceSampleCountFlagBits=TVkSampleCountFlagBits(VK_SAMPLE_COUNT_1_BIT) then begin
   if fInstance.ZFar<0.0 then begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_reversedz_loopoit_pass2_frag.spv');
   end else begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_loopoit_pass2_frag.spv');
   end;
  end else begin
   if fInstance.ZFar<0.0 then begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_reversedz_msaa_loopoit_pass2_frag.spv');
   end else begin
    Stream:=pvScene3DShaderVirtualFileSystem.GetFile('particle_msaa_loopoit_pass2_frag.spv');
   end;
  end;
 end;
 try
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass2RenderPass.ReleasePersistentResources;
begin
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass2RenderPass.AcquireVolatileResources;
...
begin
 ...
 fPassVulkanDescriptorPool.AddDescriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER,3*fInstance.Renderer.CountInFlightFrames);
 ...
 for InFlightFrameIndex:=0 to FrameGraph.CountInFlightFrames-1 do begin
  ...
  fPassVulkanDescriptorSets[InFlightFrameIndex].WriteToDescriptorSet(9,
                                                                       ...
                                                                       TVkDescriptorType(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER),
                                                                       [],
                                                                       [fInstance.LoopOrderIndependentTransparentUniformVulkanBuffer.DescriptorBufferInfo],
                                                                       [],
                                                                       false);
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass2RenderPass.ReleaseVolatileResources;
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass2RenderPass.Update(const aUpdateInFlightFrameIndex,aUpdateFrameIndex:TpvSizeInt);
begin
 inherited Update(aUpdateInFlightFrameIndex,aUpdateFrameIndex);
end;

procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass2RenderPass.OnSetRenderPassResources(const aCommandBuffer:TpvVulkanCommandBuffer;
                                                                                                           const aPipelineLayout:TpvVulkanPipelineLayout;
                                                                                                           const aRendererInstance:TObject;
                                                                                                           const aRenderPass:TpvScene3DRendererRenderPass;
                                                                                                           const aPreviousInFlightFrameIndex:TpvSizeInt;
...
procedure TpvScene3DRendererPassesLoopOrderIndependentTransparencyPass2RenderPass.Execute(const aCommandBuffer:TpvVulkanCommandBuffer;
